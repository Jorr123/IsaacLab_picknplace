...
from stack_cfg3 import StackEnvCfg
...
def main():
    ...
    while simulation_app.is_running():
        ...
        #     action[0, 2] = (cube_pos[2] + 0.15 + z_random - hand_pos[2]) * 2.0
            
        #     if dist_to_cube_xy < 0.015 and abs((cube_pos[2] + 0.15) - hand_pos[2]) < 0.02:
        #         phase = 1
        if phase == 0:
            action[0, 6] = 1.0  
            action[0, 0:2] = (cube_pos[:2] - hand_pos[:2] + xy_random) * 2.0
            
            # CHANGE: Increase 0.15 to 0.30 for a higher approach
            target_height = cube_pos[2] + 0.175 
            action[0, 2] = (target_height + z_random - hand_pos[2]) *1.1
            
            # CHANGE: Update the exit condition to match the new height
